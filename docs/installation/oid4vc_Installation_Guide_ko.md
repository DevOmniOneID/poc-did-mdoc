...
> **저장소 상태(2026-06-19):** 현재 스냅샷에는 문서와 생성된 서버 캐시 파일만 있으며, 완전한 서버 및 모바일 애플리케이션 소스 코드는 포함되어 있지 않습니다. 아래 명령은 프로젝트 구조에 표시된 디렉터리가 포함된 전체 소스 배포본을 기준으로 하며, 현재 스냅샷만으로는 실행할 수 없습니다.
...
| 서버 | 설명 | 문서 |
| :--- | :--- | :--- |
| **Issuer Server** | OID4VCI 표준에 따른 VC 발급 서버 | 현재 스냅샷에 미포함 |
| **Authorization Server** | OAuth 2.0 기반 인증/인가 서버 | 현재 스냅샷에 미포함 |
| **Verifier Server** | OID4VP 표준에 따른 VP 검증 서버 | 현재 스냅샷에 미포함 |
