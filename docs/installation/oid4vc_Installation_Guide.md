...
> **Repository status (2026-06-19):** The current snapshot contains documentation and generated server cache files, but not the complete server or mobile application source code. The commands below apply to a full source distribution containing the directories shown in the project structure and cannot be executed from this snapshot alone.
...
| :--- | :--- | :--- |
| **Issuer Server** | VC issuance server according to OID4VCI standard | Not included in the current snapshot |
| **Authorization Server** | OAuth 2.0 based authentication/authorization server | Not included in the current snapshot |
| **Verifier Server** | VP verification server according to OID4VP standard | Not included in the current snapshot |
